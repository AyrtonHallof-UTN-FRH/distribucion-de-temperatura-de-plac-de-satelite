...
L = st.sidebar.number_input(
    "Lado de la placa (m)", min_value=0.001, max_value=100.0, value=0.1
)

k = st.sidebar.number_input(
    "Conductividad térmica k (W/m·C°)", min_value=0.001, max_value=500.0, value=130.0
)  #Aluminio 7075-T6

T_borde = st.sidebar.number_input("Temperatura de borde por radiador (°C)", min_value=-273.0, max_value=1000.0, value=25.0)
...
cant_componentes = st.sidebar.number_input(
    "Cantidad de Componentes", min_value=1, max_value=25, value=4, step=1
)
...
if ejecutar_simulacion:
  ...
  for comp in componentes:
          # Calculamos el ancho del parche específico para este componente
      ...
      if "2do Orden" in esquema:
        ...
        for i in range(1, N - 2):
          for j in range(1, N - 2):
              T[i, j] = 0.5 * (2.0 * T[i + 1, j] - T[i + 2, j] + 2.0 * T[i, j + 1] - T[i, j + 2] - (dx**2 / k) * Q[i, j])
        for i in range(1, N - 1):
          for j in range(1, N - 1):
            if i == N - 2 or j == N - 2:
              T[i, j] = 0.5 * (2.0 * T[i - 1, j] - T[i - 2, j] + 2.0 * T[i, j - 1] - T[i, j - 2] + (dx**2 / k) * Q[i, j])
