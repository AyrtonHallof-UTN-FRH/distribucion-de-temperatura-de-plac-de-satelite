...
L = st.sidebar.number_input(
    "Lado de la placa (m)", min_value=0.1, value=0.5
)
...
N = st.sidebar.slider(
    "Nodos por lado (N x N)", min_value=10, max_value=100, value=30, step=10
)
...
cant_componentes = st.sidebar.number_input(
    "Cantidad de Componentes", min_value=1, max_value=10, value=4, step=1
)

componentes = []
# Posiciones por defecto predefinidas para no arrancar en cero
pos_defecto = [(70.0, 25.0), (70.0, 75.0), (25.0, 25.0), (25.0, 75.0)]

for idx in range(int(cant_componentes)):
    with st.sidebar.expander(f"Componente {idx + 1}", expanded=(idx == 0)):
        p_w = st.number_input(
            f"Potencia (W) #{idx + 1}", value=15.0, min_value=0.0, key=f"p_{idx}"
        )
        def_x, def_y = pos_defecto[idx] if idx < len(pos_defecto) else (50.0, 50.0)
        ...
        componentes.append({"potencia": p_w, "x_pct": px, "y_pct": py})
...
# Tamaño del parche de disipación (en % de nodos de la malla)
ancho_parche = max(1, int(N * 0.15))
...
for it in range(int(max_iter)):
    ...
    if "2do Orden" in esquema:
        ...
        # 2. Capa adyacente al borde (i=1 o j=1) -> Conexión con Dirichlet
        for i in range(1, N - 1):
            for j in range(1, N - 1):
                if i == 1 or j == 1:
                    T[i, j] = 0.25 * (
    ...
    # 1. Dominio interior profundo (requiere 2 nodos libres hacia cada lado)
