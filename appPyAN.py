...
T_radiador = st.sidebar.number_input("Temperatura de borde por radiador (°C)", value=25.0)
...
for idx in range(int(cant_componentes)):
    with st.sidebar.expander(f"Componente {idx + 1}", expanded=(idx == 0)):
        ...
        # NUEVO: Input para el tamaño del parche (porcentaje del lado L)
...
# NUEVO: Botón de ejecución en la barra lateral
st.sidebar.markdown("---")
...
if ejecutar_simulacion:
  ...
  with tab3:
      ...
      fig_err, ax_err = subplots(figsize=(8, 4))
      ax_err.loglog(
